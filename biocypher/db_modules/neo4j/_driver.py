...
from neo4j_utils import Driver as Neo4jUtilsDriver
from ..._create import BioCypherEdge, BioCypherNode
from ..._logger import logger
from ..._driver import Driver
from ..._config import config as _config
from ... import _misc
from ._writer import Neo4jBatchWriter

logger.debug(f'Loading module {__name__}.')


class Neo4jDriver(Driver, Neo4jUtilsDriver):
    """
    ...
    def __init__(
            self,
            # driver: Optional['neo4j.Driver'] = None, # is this used anywhere?
        ...
        delimiter = delimiter or _config('neo4j_delimiter')
        array_delimiter = array_delimiter or _config('neo4j_array_delimiter')
        quote_char = quote_char or _config('neo4j_quote_char')
            
        # initialize the online database driver
        Neo4jUtilsDriver.__init__(**locals())

        # pass arguments to the driver
        self = super(Neo4jDriver, self).__init__(
            db_name=db_name,
            db_uri=db_uri,
            db_user=db_user,
            db_passwd=db_passwd,
    ...
    def start_batch_writer(self) -> None:
        ...
        if not self.batch_writer:
            self.batch_writer = Neo4jBatchWriter(
    ...
    def __repr__(self):
        return f'<BioCypher {Neo4jUtilsDriver.__repr__(self)[1:]}'
